      ...
      <div className="container flex justify-between">
        <div className="base_info flex space-x-2 items-center">
          ...
          <div className="flex flex-col">
            <Link className="text-2xl font-bold" href={`/profile/${post_data.user.id}`}>
              {post_data.user.fullname}
            </Link>
            <span className="create_date text-[12px] font-thin">
              {new Date(post_data.create_date).toLocaleDateString()}
            </span>
          </div>
        </div>
