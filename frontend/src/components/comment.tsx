  ...
  return (
    <div className="flex items-center gap-2" key={comment_data.id} id={comment_data.id.toString()}>
      ...
      <div className="bg-muted px-3 py-1 rounded-lg">
        <Link href={`/profile/${comment_data.user.id}`} className="font-bold">
...
const CommentPart = ({ post_id, post_user_id }: { post_id: number; post_user_id: number }) => {
    ...
    const fetchComment = async () => {
      try {
        const data = await fetchAPI(
          `/comment/${post_id}?limit=${limitOffset.limit}&offset=${limitOffset.offset}`,
          'GET'
        );
        ...
        setLimitOffset((prev: any) => ({ ...prev, offset: prev.offset + prev.limit }));
      } catch (_e) {
  ...
  return (
    <div className="flex flex-col gap-2 ">
      ...
      <div
        className={cn(
          'space-y-2 text-sm text-primary-foreground/70 hover:text-primary-foreground cursor-pointer',
          hasMore ? '' : 'hidden'
        )}
