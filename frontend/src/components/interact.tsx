...
import { EmoteType } from '@/type/type';
import {
  CloudRainWind,
  Forward,
  MessageCircle,
...
const InteractPart = ({
  ...
  const handleEmote = async (type: string) => {
    try {
      ...
      await fetchAPI('/emote', 'POST', emote_data);
    ...
    <div>
      ...
      <ul className="function-button flex justify-between flex-wrap">
        <li className="flex space-x-2 items-center rounded-sm relative group">
          {!currenEmote && (
            <Button variant="ghost" onClick={() => handleEmote('hot')}>
              <Thermometer />
          ...
          <ul className="absolute opacity-0 group-hover:opacity-100 flex flex-col p-2 bg-muted rounded-full duration-300 delay-100 right-full bottom-0">
            <li
              className="hover:bg-accent p-2 rounded-full text-center"
              onClick={() => handleEmote('hot')}
            >
              <Sun size={15} />
            </li>
            <li
              className="hover:bg-green-500 p-2 rounded-full text-center"
              onClick={() => handleEmote('cool')}
            >
              <Wind size={15} />
            </li>
            <li
              className="hover:bg-secondary p-2 rounded-full text-center"
              onClick={() => handleEmote('sad')}
            >
              <CloudRainWind size={15} />
            </li>
            <li
              className="hover:bg-blue-200 p-2 rounded-full text-center"
              onClick={() => handleEmote('freeze')}
            >
              <Snowflake size={15} />
            </li>
          </ul>
        ...
        <li className="flex space-x-2 items-center rounded-sm ">
          <Button variant="ghost" onClick={() => router.push(`/post/${post_id}`)}>
            <MessageCircle />
