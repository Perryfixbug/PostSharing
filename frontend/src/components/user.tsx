...
import { newMessageType, PostType, UserType } from '@/type/type'
...
const User = ({user_data}: {user_data: UserType}) => {
    ...
    const {setPartners} = useChat()

    const handleAddFriend = async ()=>{
        const data = await fetchAPI(`/friendship/${user_data.id}`, "POST")
        console.log(data)
    }
    ...
    return (
        <div className="container flex py-5 justify-between items-center flex-wrap">
            ...
            {isAuth && <div className='flex items-center gap-2'>
                <Button 
                variant={"default"}
                ...
                disabled={userInfo.id == user_data.id || userInfo?.list_friend.find((friend_id: number)=>friend_id == user_data.id)}
                >
                    <UserPlus />
                    Add friend
                </Button>
