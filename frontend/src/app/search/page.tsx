  ...
  return (
    <div className= 'container flex flex-col py-5 px-10 space-y-2 bg-primary rounded-md' >
      ...
      <div className='slection-group flex gap-2 items-center'>
        <div 
          className= {cn('w-18 rounded-full border-2 text-center p-1 cursor-pointer', type===""?"bg-secondary text-secondary-foreground":"")}
          onClick={()=>{setType("")}}
        >All</div>
        <div 
          className= {cn('w-18 rounded-full border-2 text-center p-1 cursor-pointer', type==="post"?"bg-secondary text-secondary-foreground":"")}
          onClick={()=>{setType("post")}}
        >Post</div>
        <div 
          className={cn('w-18 rounded-full border-2 text-center p-1 cursor-pointer', type==="people"?"bg-secondary text-secondary-foreground":"")}
      ...
      <p>Result</p>
      {searchResult?.user && 
      searchResult.user.map((user: UserType)=>(
        <User key={user.id} user_data={user}/>
      ))
      }
      {searchResult?.post && 
