      ...
      <div className="slection-group flex gap-2 items-center">
        <div
          className={cn(
            ...
            type === 'pending' ? 'bg-secondary text-secondary-foreground' : ''
          )}
          onClick={() => {
            setType('pending');
          }}
        ...
        <div
          className={cn(
            'w-18 rounded-full border-2 text-center p-1 cursor-pointer',
            type === 'accepted' ? 'bg-secondary text-secondary-foreground' : ''
          )}
          onClick={() => {
            setType('accepted');
          }}
      ...
      {searchResult &&
        searchResult.map((friend) => {
          if (type == 'pending') return <Friend key={friend.friend_id} friend_data={friend.info} />;
            ...
            return <User key={friend.friend_id} user_data={friend.info} />;
        })}
    </div>
  ) : (
    <AuthFirst />
