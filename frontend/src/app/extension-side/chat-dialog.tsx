...
import {Card, CardTitle, CardHeader, CardContent, CardAction, CardFooter} from '@/components/ui/card'
import { useAuth } from '@/context/authContext'
import { fetchAPI } from '@/lib/api'
import { Circle, CircleSmall, CircleSmallIcon, MessageCircle, Send, X } from 'lucide-react'
...
const PreviewChat = ({partner, historyChat, setChatUsers}: {partner: UserType, historyChat: MessageType[], setChatUsers: any})=>{
  ...
  return(
    <div>
      <div 
        ...
        onClick={() => setChatUsers((prev: number[]) => {
...
const Chat = ({partner, historyChat, setChatUsers}: {partner: UserType, historyChat: MessageType[], setChatUsers: any})=>{
    ...
    <Card className='w-80'>
      <CardHeader className='flex items-center justify-between'>
        ...
        <Button 
          variant={"ghost"} 
          className=''
          onClick={()=>setChatUsers((prev: any)=>(
            prev.filter((item: any)=>item !== partner.id)
          ))}
        >
          <X />
        </Button>
      ...
      <CardContent className='overflow-y-auto h-[300px]' ref={scrollRef}>
        <div className='flex flex-col gap-1'>
          {historyChat && historyChat.map((messageData)=>(
...
const ChatDialog = ({option, setOption}: {option: string, setOption: any}) => {
  const {partners, allHistoryChat, setPartners, setAllHistoryChat} = useChat()
  ...
  return (
    <div className=''>
        <Button 
          size="icon" 
          className={cn('rounded-full relative', option == Options.CHAT ? "bg-accent text-accent-foreground hover:bg-accent/80" : "")}
          onClick={()=>{
        ...
        >
          ...
          {newMessageCount > 0 && <Badge className='absolute top-[-5px] right-[-5px] rounded-full bg-destructive'>{newMessageCount}</Badge>}
        </Button>
        {option === Options.CHAT && 
          <Card className='md:absolute inset-x-0 md:h-[86vh] my-2 fixed h-full'>
            ...
            <CardContent className='flex flex-col'>
              {partners && partners.map((partner: UserType)=>(
                <PreviewChat 
                  key={partner.id} 
                  partner={partner}
