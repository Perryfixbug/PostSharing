...
const PreviewNoti  = ({noti}: {noti: NotiType}) =>{
  ...
  const {allNoti, setAllNoti} = useNoti()

  const handleReadNoti = async ()=>{
    const data = await fetchAPI(`/notification/${noti.id}`, "PUT")
    console.log(data);
    
    setAllNoti((prevNoti: NotiType[]) =>
      prevNoti.map((n: NotiType) =>
  ...
  return(
    <div 
      ...
      className='relative cursor-pointer hover:bg-muted p-2 rounded-lg'
    >
        {noti.is_read ? 
          <div>
            <p className='text-[12px] font-light'>{(new Date(noti.create_date)).toLocaleTimeString()}</p>
            <p className=''>{noti.content}</p>
          </div>  
          :
          <div>
            <p className='text-[12px] font-medium'>{(new Date(noti.create_date)).toLocaleTimeString()}</p>
...
const NotificationDialog = ({option, setOption}: {option: string, setOption: any}) => {
  ...
  return (
    <div>
        <Button 
          size="icon" 
          className={cn('rounded-full relative', option == Options.NOTI ? "bg-accent text-accent-foreground hover:bg-accent/80" : " ")}
          onClick={()=>{
        ...
        >
          <Bell className="" />
          {newNotiCount > 0 && <Badge className='absolute top-[-5px] right-[-5px] rounded-full bg-destructive'>{newNotiCount}</Badge>}
        </Button>
        {option === Options.NOTI && 
        <Card className='md:absolute inset-x-0 md:h-[86vh] my-2 fixed h-full'>
          ...
          <CardContent className='overflow-y-auto max-h-[500px]'>
            <div className='flex flex-col gap-1'>
              {allNoti.length > 0 && allNoti.map((noti: NotiType)=>(
                <PreviewNoti key={noti.id} noti={noti} />
              ))}
            </div>
          </CardContent>
        </Card>
